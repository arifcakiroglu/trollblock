...
chrome.tabs.onUpdated.addListener(function (tabId, changeInfo, tab) {
  ...
  chrome.tabs.executeScript(tabId, {
    code: 'var injected = window.trollblockInjected; window.trollblockInjected = true; injected;',
    runAt: 'document_start'
  }, function (res) {
    if (chrome.runtime.lastError ||res[0])
      return;

    var cssFiles = ['styles/trollblock.css'];

    var jsFiles = ['scripts/jquery.js', 'scripts/trollblock.js'];
  ...
  var handler = {
    requestPermissions: function requestPermissions() {
      ...
      function removeUnnecessaryPermissions() {
        var whitelist = urls.concat(['https://eksisozluk.com/*']);
